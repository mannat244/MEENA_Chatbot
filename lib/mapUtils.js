...
export const renderTextWithMaps = (text, InteractiveMapComponent) => {
  ...
  // Sort matches by index in ascending order for proper sequential processing
  mapMatches.sort((a, b) => a.index - b.index);
  ...
  mapMatches.forEach((match, index) => {
    // Create simple, sequential placeholder
    const mapPlaceholder = `[MAP_${index}]`;
    
    // Calculate adjusted position considering previous replacements
    const adjustedIndex = match.index + offset;
    
    // Replace the MAP_COORDINATES{} with placeholder
    processedText = processedText.substring(0, adjustedIndex) + 
                   mapPlaceholder + 
                   processedText.substring(adjustedIndex + match.fullMatch.length);
    
    // Update offset for next replacement
    offset += mapPlaceholder.length - match.fullMatch.length;
    
    // Create React component for rendering
    const mapComponent = React.createElement(InteractiveMapComponent, {
      key: `map_${index}`,
